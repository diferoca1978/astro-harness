...
Read `specs/TEMPLATE.md` to see the full structure the spec will follow —
section order, headings, and header format. `template.md` (in the same
directory as this skill) is a companion quality guide for *how* to fill
those sections well; it is not a second source of structure. Lean on both
at every step.
...
In both cases the content follows `specs/TEMPLATE.md`'s order:

1. **Header** (just the `**Status:** Draft` line — no dependency or date
   fields).
2. **Goal** (one or two sentences: what this adds or changes, and why it's
   needed now). Critical — if it does not fit in one or two sentences, go
   back to Phase 2.
3. **Scope** (`**In:**` / `**Out:**`, both explicit).
4. **Files affected** (concrete paths, cross-referenced against
   `AGENTS.md`'s "If you change X → update Y" table — name the row this
   spec matches, or say it matches none).
5. **Decisions made and discarded** (with brief justification for each).
6. **Acceptance criteria** (boolean checklist, not aspirational).
7. **Implementation plan** (numbered steps, each leaving the system
   functional).
...
3. Write the file directly at `specs/NN-slug.md` with all the sections. **Do not ask for permission to write it and do not ask whether the file name works** — announce the path in the final confirmation. Only ask if the target file already exists.
4. Mark the state as `Draft` by default (or the equivalent word used by the existing specs in this repo). **Do not mark it as `Approved` automatically** — the user does that once they have re-read it.
5. If the Goal or Files-affected prose names another spec as a
   dependency, check that spec actually exists in `specs/`. If it does
   not, say so instead of writing a dangling reference.
6. **Seed the config file if it does not exist.** Check for `specs/.spec-config.yml`. If it is **missing**, create it with the default content below. If it **already exists, leave it untouched** — never overwrite the user's settings.
   ...
   #   true  (default) → /spec-impl creates and switches to the branch without asking
   #   false           → /spec-impl asks for [y/N] confirmation before creating the branch
   AutoCreateBranch: true
   #
   # BranchPrefix — prefix /spec-impl uses when deriving the branch name from NN-slug.
   #   feature/ (default) → NN-slug becomes feature/NN-slug
   #   ""                 → no prefix, the branch is the bare NN-slug
...
7. Confirm to the user:
   ...
   - If you just created `specs/.spec-config.yml`, mention it exists and that `AutoCreateBranch` defaults to `true` and `BranchPrefix` defaults to `feature/` (set either to change the behavior).
